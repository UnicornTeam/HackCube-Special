...
void AddRfList(String data) {
    ...
    for (int i = 0; i < RFnum; i++) {
        if (RFlist[i].data == data.substring(data.indexOf("data") + 5, data.indexOf('\n') - 1) &&
            RFlist[i].func == data.substring(data.indexOf("func:") + 5, data.indexOf(",data")))
            return;
    }
...
void Add_NFC(String data) {
    ...
    for (int i = 0; i < NFCnum; i++) {
        if (NFClist[i].data == strtoul(data.substring(data.indexOf("data") + 5, data.length()).c_str(), NULL, 10) &&
            NFClist[i].vd == atoi(data.substring(data.indexOf("vd") + 3, data.indexOf(",data")).c_str()))
            return;
    }
