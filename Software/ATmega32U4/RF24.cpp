...
/****************************************************************************/
uint8_t RF24::Reader(void *buf) {
    uint8_t rx_len = read_register(R_RX_PAYLOAD);
...
//	NRF_CE_L();	
//	read_register(R_RX_PAYLOAD,buf,5);
//	NRF_CE_H();
    read_payload(buf, 5);
...
void RF24::openWritingPipe(const uint8_t *address, int len) {
    // Note that AVR 8-bit uC's store this LSB first, and the NRF24L01(+)
    // expects it LSB first too, so we're good.

    write_register(RX_ADDR_P0, address, len);
    write_register(TX_ADDR, address, len);
